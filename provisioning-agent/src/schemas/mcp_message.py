...
class ProvisioningTaskResult(BaseModel):
    """Provisioning-specific task result."""
    action: Literal["task_result"] = "task_result"
    workflow_id: str
    tenant_id: str
    task_id: str
    status: Literal["success", "failed"]
    result: Dict[str, Any] = Field(default_factory=dict, description="Result data including external_id, reference_url, details")
    error: Optional[str] = None
    retryable: bool = Field(default=True, description="Whether task can be retried")
